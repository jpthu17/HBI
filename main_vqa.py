...
from HBI.models.file_utils import PYTORCH_PRETRAINED_BERT_CACHE
from HBI.models.modeling_qa import HBI
from HBI.models.optimization import BertAdam
from torch.utils.data import DataLoader
from HBI.utils.util import parallel_apply, get_logger
from HBI.dataloaders.dataloader_msrvtt_qa import MSRVTT_DataLoader, MSRVTT_TrainDataLoader
from HBI.utils.metrics_qa import accuracy, AverageMeter
...
def get_args(description='Video-Text as Game Players: Hierarchical Banzhaf Interaction for Cross-Modal Representation Learning'):
    ...
    parser.add_argument('--sim_header', type=str, default="meanP",
                        choices=["meanP", "seqLSTM", "seqTransf", "tightTransf", "BTransf", "denseTransf"],
                        help="choice a similarity header.")
    parser.add_argument('--loss', type=str, default="CrossEn",
                        choices=["CrossEn", "DualLoss", "CrossCLR", "MarginLoss", "DCWLoss"])
...
def dataloader_msrvtt_train(args, tokenizer):
    msrvtt_dataset = MSRVTT_TrainDataLoader(
        jsonl_path=args.train_csv,
        ans2label_path=args.data_path,
...
def dataloader_msrvtt_test(args, tokenizer):
    msrvtt_testset = MSRVTT_DataLoader(
        jsonl_path=args.val_csv,
        train_jsonl=args.train_csv,
        ans2label_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
        feature_framerate=args.feature_framerate,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info("Epoch: %d/%s, Step: %d/%d, Lr: %s, CeLoss: %f, Time/step: %f", epoch + 1,
                            args.epochs, step + 1,
    ...
    if args.do_train:
        ...
        for epoch in range(args.epochs):
            ...
            tr_loss, global_step = train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
                                               scheduler, global_step, local_rank=args.local_rank, tokenizer=tokenizer)
            if args.local_rank == 0:
